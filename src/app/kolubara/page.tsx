      ...
      <div className="relative z-10">
        <header className="border-b border-border">
          <div className="container mx-auto px-4 py-3 md:py-4 flex items-center justify-between">
            <Link href="/" className="flex items-center gap-1 md:gap-2 text-foreground hover:text-muted-foreground transition-colors">
        ...
        <main className="container mx-auto px-4 py-12 md:py-20">
          <section className="max-w-4xl mx-auto space-y-6 md:space-y-10">
            <div className="text-center space-y-4 md:space-y-6">
              ...
              <p className="text-base md:text-xl text-muted-foreground">Слушајте уживо Радио Колубару</p>
            </div>
            ...
            <div className="bg-card border border-border rounded-lg md:rounded-xl p-6 md:p-10 shadow-md relative overflow-hidden">
                ...
                <div className="absolute inset-0 opacity-10 pointer-events-none">
                  <div className="absolute bottom-0 left-0 right-0 h-28 md:h-40 flex items-end justify-around">
                    {[...Array(20)].map((_, i) => (
                      <div 
                        key={i}
                        className="w-0.5 bg-red-500 rounded-t-full audio-bar"
                        style={{
                          height: '5%',
              ...
              <div className="space-y-6 md:space-y-8 relative z-10">                
                <div className="flex items-center justify-center gap-4 mt-2 md:mt-4">
                  <Button 
                    ...
                    className={`h-14 w-14 md:h-16 md:w-16 rounded-full shadow-sm transition-all ${
                      isPlaying ? 'border-red-500 shadow-red-500/10 bg-red-500/5' : ''
                    }`}
                  ...
                  >
                    {isLoading ? (
                      ...
                      <Pause className={`h-7 w-7 md:h-8 md:w-8 ${isPlaying ? 'text-red-500' : ''}`} />
                    ) : (
                ...
                <div className="mt-6 md:mt-8">
                  ...
                  {infoOpen && (
                    <div className="text-center text-sm md:text-base text-muted-foreground mt-4 pt-4 border-t border-border animate-accordion-down">
                      <p>Стрим се емитује директно са сервера Радио Колубаре.</p>
                      <p className="mt-2">У случају проблема, освежите страницу или проверите вашу интернет конекцију.</p>
                    </div>
                  )}
                </div>
              </div>
