    ...
    <div className="overflow-hidden h-screen w-screen">
      <style jsx global>{`
        @keyframes flashColors {
          ...
          10% { background-color: #ff7700; }
          20% { background-color: #ffff00; }
          30% { background-color: #77ff00; }
          40% { background-color: #00ff00; }
          50% { background-color: #00ff77; }
          60% { background-color: #0000ff; }
          70% { background-color: #7700ff; }
          80% { background-color: #ff00ff; }
          90% { background-color: #ff0077; }
      ...
      <div className="flashing-bg h-full w-full flex items-center justify-center">
        <div className={`responsive-text font-black text-white text-center px-4 max-w-full break-words ${libreBaskerville.className}`}>
          ВРТЛОЖНИЦА<br />ЈЕ ПУНА
        </div>
      </div>
    </div>
  );
